...
Next time you're tempted to type \`z-index: 99999\`, pause. Open DevTools. Check the parents. The answer is almost always there.
  ...
  {
    id: 'debugging-missing-semicolon',
    title: 'I Spent 3 Hours Debugging. It Was a Missing Semicolon.',
    excerpt:
...
It was a Friday afternoon (of course it was). I was adding a new feature to a React component — nothing fancy, just a small state update triggered by a button click. The component renders. The button renders. I click it. Nothing happens.

No error in the console. No warning. Just... silence.

## The Investigation

I started where every developer starts: \`console.log\` everywhere.

\`\`\`js
// what the code looked like — console.log as my only debugging tool
const handleClick = () => {
  console.log('clicked')       // ✅ this printed fine
  updateState(newValue)        // 💀 silently died here
  console.log('state updated') // never reached
}
\`\`\`

Wait. The function stops executing halfway through? No error thrown? That's... weird.

I checked the \`updateState\` function. Looked fine. I checked the import. Fine. I checked if something was swallowing errors somewhere. Nothing obvious.

## The Rabbit Hole

An hour in, I started suspecting the build. Maybe something wasn't compiling correctly. I cleared the cache, restarted the dev server, did the classic "turn it off and on again."

Still broken.

I started reading about React state batching. Maybe the update was being swallowed silently in some edge case? I read three articles. None of them helped.

## The Actual Problem

Two and a half hours in, completely out of ideas, I decided to do a full project search for the function name. And there it was — in a utility file I hadn't touched in weeks:

\`\`\`js
// Run this — processValue never executes, and there's NO error thrown
// This is why silent failures are the worst kind of bug

function processValue(val) {
  console.log('processValue ran:', val)
}

// The "broken" export — early return swallows execution silently
const updateState = (val) => {
  if (!val) return  // if val is falsy, we bail out here silently
  processValue(val)
}

// This will print nothing — updateState receives undefined
// and silently returns without calling processValue
updateState(undefined)
console.log('done — but processValue never ran 👆')

// Fix: pass an actual value
updateState('hello') // now processValue runs
\`\`\`

Except it wasn't actually a missing semicolon in that spot. It was in the line *above* it, in a different function, that had caused a cascading parse issue that silently broke the export.

The kind of bug that makes you stare at the wall for five minutes after fixing it.

## What I Learned

- Always check the file the function *lives in*, not just where it's *called*
- "No error" doesn't mean "no problem" — silent failures are the worst kind
- Linters exist for a reason. ESLint with strict settings would have caught this immediately
- Take a break after an hour of no progress. Fresh eyes find things tired eyes miss

You'll have days like this. We all do. The semicolon wins sometimes. But now you know to look for it.`,
  },
  {
    id: 'stop-using-px',
